...
public class Interfaz extends javax.swing.JFrame {    
    ...
    public Interfaz() {
        ...
        this.setTitle(".....:::::Algoritmo Colas Multiples Retroalimentadas 1.0:::::.....");
        initComponents();
    ...
    public void compararE (){
        if(Proc.p.sig.tiempo!=0&&Proc.p.sig.proceso!=0){
            if((Proc.p.sig.tiempo<en_ejecucionE.tiempo)&&(Proc.p.sig.prioridad==2)){
                    ...
                    Proc.nuevoSE.envejecimiento=en_ejecucionE.envejecimiento;
    ...
    public void ordenar (nodo nuevo, nodo cab, nodo p){
        ...
        while(nuevo!=p){
            ...
            b[contor][4]=nuevo.envejecimiento;  
            nuevo=nuevo.sig;
            contor=contor+1;
        }            
        ...
        while(nuevo!=p){
            ...
            nuevo.envejecimiento=b[contor][4]; 
            nuevo=nuevo.sig;
            contor=contor+1;
        }    
    }
    ...
    public void Sistema(){//roundrobin
        ...
        if(Proc.pSS != Proc.pSS.sig){ 
            ...
            if(Proc.nuevoSS.TSUSP == 1){              
                ...
                Proc.nuevo.envejecimiento=Proc.nuevoSS.envejecimiento;
        ...
        if(Proc.pBS != Proc.pBS.sig){
            ...
            if(Proc.nuevoBS.TBLOQ == 1){                
                ...
                Proc.nuevo.envejecimiento=Proc.nuevoBS.envejecimiento;
                if(Proc.nuevo.tiempo<=Proc.promedioCPU){
                    Proc.nuevo.QTM = Proc.nuevo.tiempo;
                }else{
                    Proc.nuevo.QTM = Proc.promedioCPU+(Proc.promedioCPU / Proc.promedioCPU);
        ...
        if(en_ejecucionS != null){
            if(en_ejecucionS.QTM == 1){                
                ...
                if(en_ejecucionS.tiempo == 1){
                    ...
                    Proc.nuevoSS.envejecimiento=en_ejecucionS.envejecimiento;
                    int x=0;
                    double y=0;
                    while (x<2){
        ...
        if(en_ejecucionS== null){
            ...
            if (bloq==1&&en_ejecucionS!=null){
                if(TxtArea_BloqueadoSistema.getText().equals("") && TxtArea_en_ejecucionSistema.getText().equals("") && TxtArea_Listo.getText().equals("")){}
                        ...
                        Proc.nuevoBS.envejecimiento=en_ejecucionS.envejecimiento; 
                        int x=0;
                        double y=0;
                        while (x<2){
                            y =  (((int) Math.round((Math.random()*100)))*(4))/100;
    ...
    public void EntradaSalida(){//srtf
        ...
        if(Proc.pSE != Proc.pSE.sig){
            ...
            if(Proc.nuevoSE.TSUSP == 1){ 
                ...
                Proc.nuevo.envejecimiento=Proc.nuevoSE.envejecimiento;
                Proc.cab.sig=Proc.nuevo;
                Proc.nuevo.sig=Proc.p; 
                Proc.cab=Proc.nuevo;                
                TxtArea_Listo.setText("");                
        ...
        if(Proc.pBE != Proc.pBE.sig){ 
            ...
            if(Proc.nuevoBE.TBLOQ == 1){ 
                ...
                Proc.nuevo.envejecimiento=Proc.nuevoBE.envejecimiento;
                Proc.cab.sig=Proc.nuevo;
                Proc.nuevo.sig=Proc.p; 
                Proc.cab=Proc.nuevo;                
                TxtArea_Listo.setText("");                
            ...
            if(en_ejecucionE!=null){
                if(en_ejecucionE.tiempo == 1){
                    ...
                    if(aux==aux.sig){
                        ...
                        en_ejecucionE.envejecimiento = en_ejecucionE.envejecimiento;
                        TxtArea_en_ejecucionEntradaSalida.setText("");
        ...
        if (bloq==1&&en_ejecucionE!=null){
            if(TxtArea_BloqueadoEntradaSalida.getText().equals("") && TxtArea_en_ejecucionEntradaSalida.getText().equals("") && TxtArea_Listo.getText().equals("")){}
                    ...
                    Proc.nuevoBE.envejecimiento=en_ejecucionE.envejecimiento;                    
                    int x=0;
                    double y=0;
                    while (x<2){
                        y =  (((int) Math.round((Math.random()*100)))*(4))/100;
    ...
    public void Usuario(){//FIFO
        ...
        if(Proc.pBU != Proc.pBU.sig){ 
            ...
            if(Proc.nuevoBU.TBLOQ == 1){ 
                ...
                Proc.nuevo.envejecimiento=Proc.nuevoBU.envejecimiento;
                Proc.cab.sig=Proc.nuevo;
                Proc.nuevo.sig=Proc.p; 
                Proc.cab=Proc.nuevo;                
                TxtArea_Listo.setText("");                
            ...
            if(en_ejecucionU!=null){
                if(en_ejecucionU.tiempo == 1){
                    ...
                    if(aux==aux.sig){
                        ...
                        en_ejecucionU.envejecimiento = en_ejecucionU.envejecimiento;
                        TxtArea_en_ejecucionUsuario.setText("");
        ...
        if (bloq==1&&en_ejecucionU!=null){
            if(TxtArea_BloqueadoUsuario.getText().equals("") && TxtArea_en_ejecucionUsuario.getText().equals("") && TxtArea_Listo.getText().equals("")){}
                    ...
                    Proc.nuevoBU.envejecimiento=en_ejecucionU.envejecimiento;
                    int x=0;
                    double y=0;
                    while (x<2){
                        y =  (((int) Math.round((Math.random()*100)))*(4))/100;
    ...
    public void Envejecer(nodo nuevo, nodo cab, nodo p){
        nuevo=p.sig;
        while(nuevo!=p){
            nuevo.envejecimiento=nuevo.envejecimiento-1;                        
            if(nuevo.envejecimiento==1 && nuevo.prioridad==3){
               nuevo.prioridad=2;               
               probenv = 0;
               while(probenv<30){
                   probenv=(int) Math.round((Math.random()*300));                     
                   nuevo.envejecimiento=probenv;               
               }
               JOptionPane.showMessageDialog(null, "envejecio el proceso "+nuevo.proceso+"!!! \n ahora su prioridad es "+nuevo.prioridad+" !!!");
            }
            if(nuevo.envejecimiento==1 && nuevo.prioridad==2){
               nuevo.prioridad=1;               
               if(nuevo.prioridad==1){
                    if(nuevo.tiempo<=Proc.promedioCPU){
                       nuevo.QTM = nuevo.tiempo;
                    }else{
                       nuevo.QTM = Proc.promedioCPU+(nuevo.tiempo/Proc.promedioCPU);
                    }
               }else{
                    nuevo.QTM = 0;
               }
            ...
            if(nuevo.envejecimiento==1 && nuevo.prioridad==1){
               nuevo.prioridad=nuevo.prioridad;               
               probenv = 0;
               while(probenv<30){
                   probenv=(int) Math.round((Math.random()*300));                     
                   nuevo.envejecimiento=probenv;               
    ...
    private void initComponents() {
        ...
        jLabel17.setText("Colas Multiples Retroalimentadas");
        getContentPane().add(jLabel17);
        jLabel17.setBounds(30, 20, 370, 46);
...
Timer timer = new Timer (400, new ActionListener (){ 
    public void actionPerformed(ActionEvent e){ 
        ...
        EntradaSalida();
        Usuario();
        Envejecer(Proc.nuevo, Proc.cab, Proc.p);
    }       
    }); 
        ...
        try{
            if(Integer.parseInt(jTextField3.getText())<=1000000 && 
               ...
               if(c==false){
                    ...
                    while(probenv<30){
                        probenv=(int) Math.round((Math.random()*300));                     
                        Proc.nuevo.envejecimiento=probenv;
                    }
                    Proc.cab.sig=Proc.nuevo;
                    Proc.nuevo.sig=Proc.p; 
                    Proc.cab=Proc.nuevo;
